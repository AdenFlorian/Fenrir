...
public class ActionMaster : MonoBehaviour {
	...
	void Init() {
		...
		if (!overwriteConfigFile && File.Exists(configPath)) {
			...
			Debug.Log("Keybindings.txt NOT found or set to overwrite! Generating config from default keys...");
	...
	private void AddDefaultKeyCodes() {
		...
		keyCodesFor[ActionCode.Interact].Add(KeyCodeExt.F);
		keyCodesFor[ActionCode.Interact].Add(KeyCodeExt.Mouse0);
	}

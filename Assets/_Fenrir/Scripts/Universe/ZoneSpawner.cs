...
public class ZoneSpawner : MonoBehaviour {
	...
	void SpawnUniPlayer(UniversePlayerDesc uniPlayerDesc) {
		...
		//spawnedPlayer.transform.parent = uniPlayerDesc.zoneParent.gameObject.transform;
	}
	...
	void SpawnZone(Zone zone) {
		...
		foreach (ZoneMember member in zone.zoneMembers) {
			GameObject zoneMemberGO = Instantiate(GameObject.CreatePrimitive(PrimitiveType.Sphere),
				member.innerZonePosition + (Zone.ZONE_WIDTH * zoneOffset),
				Quaternion.identity) as GameObject;
			//member.gameObject.transform.parent = zone.gameObject.transform;
		}
	}
